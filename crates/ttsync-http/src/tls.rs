...
//!
//! Loads PEM certificates or generates a long-term key + self-signed certificate,
//! and computes the SPKI SHA-256 hash for certificate pinning.
...
/// TLS identity loaded from PEM files or generated by TT-Sync.
...
impl SelfManagedTls {
    ...
    /// Read a leaf-first PEM certificate chain and its private key without modifying either file.
    pub fn from_pem_files(cert_path: &Path, key_path: &Path) -> Result<Self, SyncError> {
        let cert_pem = std::fs::read(cert_path).map_err(|e| {
            SyncError::Io(format!("read TLS certificate {}: {e}", cert_path.display()))
        })?;
        let key_pem = std::fs::read(key_path).map_err(|e| {
            SyncError::Io(format!("read TLS private key {}: {e}", key_path.display()))
        })?;
        let cert_chain = parse_cert_chain_pem(&cert_pem).map_err(|e| {
            SyncError::InvalidData(format!("TLS certificate {}: {e}", cert_path.display()))
        })?;
        let private_key = parse_private_key_pem(&key_pem).map_err(|e| {
            SyncError::InvalidData(format!("TLS private key {}: {e}", key_path.display()))
        })?;
        let tls = Self {
            spki_sha256: spki_sha256_from_cert(&cert_chain[0])?,
            cert_chain,
            private_key,
        };
        tls.server_config().map_err(|e| {
            SyncError::InvalidData(format!(
                "TLS certificate {} and private key {}: {e}",
                cert_path.display(),
                key_path.display()
            ))
        })?;
